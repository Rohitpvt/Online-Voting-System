  ...
  try {
    ...
    if (email.isNotEmpty) {
      ...
      Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => EmailOTPscreen(email: email, phone: phone,password: password, isLogin: false,firstName: firstName,lastName: lastName,dob: dateString,),
          ),
        );
      ...
      if (otpSent) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("OTP sent to your email",style: TextStyle(fontFamily: "urmed",fontSize: 17),),
      ...
      setState(() {
        ...
        // Redirect to OTP verification
            await sendOtpAndNavigate(
              ...
              phoneNumber,
              emailAddress,
              dob: dob != null ? "${dob!.day}/${dob!.month}/${dob!.year}" : "Not provided",
              isLogin: false
            );
        ...
        setState(() {
          errorMessage = "Error creating account: ${e.toString()}";
          successMessage = "";
          showProgress = false; // Stop loading animation if there’s an error
        });
      }}
                            ...
                            if (errorMessage.isNotEmpty)
                              Padding(
                                ...
                                child: Center(
                                  child: Text(
                                    errorMessage,
                                    style: const TextStyle(color: Color.fromARGB(255, 239, 20, 4) , fontFamily: "urmed", fontSize: 16),
                                  ),
                                ),
                              ),
