...
class forgotpassOTP extends StatefulWidget {
  ...
  const forgotpassOTP({super.key, required this.email, required this.phoneNumber});

  @override
  _forgotpassOTPState createState() => _forgotpassOTPState();
}

class _forgotpassOTPState extends State<forgotpassOTP> with WidgetsBindingObserver {
  ...
  Future<void> _verifyOTP() async {
    ...
    if (otp.length == 6 && await EmailOTP.verifyOTP(otp: otp)) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => ForgotPassPage(phone: widget.phoneNumber,)),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text(
  ...
  Widget build(BuildContext context) {
    ...
    const emailTextStyle = TextStyle(
      fontSize: 18,
      color: Colors.white,
      fontFamily: "urwbold", // Use your custom font family here
    );
